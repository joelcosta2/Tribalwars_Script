...
async function runAutoTrainer() {
    const userChoice = await showWarningPopup('Confirm Auto Paladin Train', 'Do you wish to continue with the paladin train automation?');
    if (userChoice === 'cancel') {
        return;
    } else {
        var trainLevel = parseInt(localStorage.getItem('auto_trainer_paladin_level'));
        if (trainLevel !== -1) {
            var trainStartButton = document.querySelector('.paladin_train_launch');
            if (trainStartButton) {
                trainStartButton.click();
                wait(1).then(() => {
                    var trainLevelsButtons = document.querySelectorAll('.btn.paladin_regimen_confirm:not([class*="btn-pp"])');
                    var trainLevel = parseInt(localStorage.getItem('auto_trainer_paladin_level'));
