...
function computeScavengeOptimizedDistribution(unitCounts, unlockedOptsData, mode) {
    ...
    let html = '<table class="vis" style="width:100%;font-size:11px;margin-top:8px;"><tr><th>Level</th>';
    unitCols.forEach(u => { html += `<th><img src="${base}unit/unit_${u}.png" style="width:20px" title="${u}"/></th>`; });
    html += '<th>Cap</th><th>Resources</th><th>Duration</th></tr>';
    unlockedOptsData.forEach((opt, i) => {
        let cap = 0;
...
async function runOptimizedScavenge(forceRun = false) {
    ...
    for (const optionId of optionIds) {
        ...
        if (!result.success) {
            ...
            if (typeof showAutoHideBox === 'function') showAutoHideBox('[Auto-Scavenge] Disabled — send failed for level ' + optionId, true);
            return;
        }
...
function injectScavengeConfigPanel() {
    ...
    optimizeSpan.textContent = 'Multi-level (optimized)';
    ...
    optimizeHint.textContent = 'Distributes troops across all unlocked levels using the balanced (max res/h) or fastest (simultaneous return) formula. The saved distribution is reused on every automatic re-send.';
    ...
    unlockedOptions.forEach((opt, i) => {
        ...
        label.appendChild(document.createTextNode(titleEl ? titleEl.textContent.trim() : 'Level ' + (i + 1)));
    ...
    mr0.textContent = 'Mode:';
    const mr1 = modeRow.insertCell(1);
    [['balanced', 'Balanced (max res/h)'], ['fastest', 'Fastest (simultaneous return)']].forEach(([val, lbl]) => {
        const label = document.createElement('label');
    ...
    fillAllLink.textContent = 'Todas as tropas';
    ...
    optimizeCheckbox.onchange = () => {
        ...
        document.getElementById('scavenge_mode_row').style.display = isOpt ? '' : 'none';
        if (!isOpt) {
    ...
    function getSelectedUnlockedOptsData() {
        const selectedIndices = levelCheckboxes.filter(cb => cb.checked).map(cb => parseInt(cb.dataset.levelIndex));
    ...
    saveBtn.onclick = async () => {
        ...
        if (isOptimize) {
            ...
            const selectedLevelIndices = levelCheckboxes.filter(cb => cb.checked).map(cb => parseInt(cb.dataset.levelIndex));
            ...
            const selectedCb = levelCheckboxes.find(cb => cb.checked) || levelCheckboxes[levelCheckboxes.length - 1];
            const selectedLevel = selectedCb ? parseInt(selectedCb.dataset.levelIndex) : unlockedOptions.length;
            saveScavengeConfig({ enabled: isEnabled, level: selectedLevel, units, optimizeMode: false, distributionByOption: null });
            if (typeof showAutoHideBox === 'function') showAutoHideBox('Configuration saved!', false);
        }

        if (isEnabled) sendNowBtn.click();
    };
    ...
    sendNowBtn.onclick = async () => {
        ...
        try {
            ...
            if (isOptimize) {
                ...
                const { distributionByOption } = computeScavengeOptimizedDistribution(units, unlockedOptsData, calcMode);
                const isEnabled = document.getElementById('scavenge_config_enabled').checked;
                const previousConfig = getScavengeConfig();
                saveScavengeConfig({ enabled: isEnabled, level: 0, units, optimizeMode: true, optimizeCalculationMode: calcMode, distributionByOption, selectedLevelIndices });
                await runOptimizedScavenge(true); // forceRun bypasses the enabled check
                // If runOptimizedScavenge disabled itself due to a send failure, restore the previous config
                if (isEnabled && !getScavengeConfig().enabled) {
                    saveScavengeConfig(previousConfig);
                }
                _scavengeSchedulePageReload();
            } else {
                // Non-optimize: send directly without going through the bot scheduler
                const selectedCb = levelCheckboxes.find(cb => cb.checked) || levelCheckboxes[levelCheckboxes.length - 1];
                const level = selectedCb ? parseInt(selectedCb.dataset.levelIndex) : unlockedOptions.length;
                const isEnabled = document.getElementById('scavenge_config_enabled').checked;

                const nonZeroUnits = Object.fromEntries(Object.entries(units).filter(([, v]) => v > 0));
                if (!Object.keys(nonZeroUnits).length) {
                    if (typeof showAutoHideBox === 'function') showAutoHideBox('No units specified.', true);
                    return;
                }
                ...
                if (!targetOpt) {
                    if (typeof showAutoHideBox === 'function') showAutoHideBox('No available scavenge option found.', true);
                    return;
                }
                ...
                if (result.success) {
                    saveScavengeConfig({ enabled: isEnabled, level, units, optimizeMode: false, allUnits: false, distributionByOption: null });
                }
                if (result.returnMs > 0) {
